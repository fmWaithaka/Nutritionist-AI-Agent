...
def analyze_image_with_rest(api_key: str, image_bytes: bytes, language: str = "English"):
    ...
    if not image_bytes:
        ...
        st.warning("No image bytes provided for analysis.") # Keep user warning
        return None
    ...
    try:
        ...
        try:
             ...
             if not result_json["candidates"][0].get("content") or not result_json["candidates"][0]["content"].get("parts"):
                  log.error("Vision Error: Unexpected content/parts structure in response.")
                  st.error("❌ Image Analysis Error: Unexpected response structure.")
                  log.error("Full API Response: %s", result_json)
                  return None

             text_result = result_json["candidates"][0]["content"]["parts"][0]["text"]
             log.info(f"Vision API Extracted Text (first 500): {text_result[:500]}")
        except (KeyError, IndexError, TypeError) as e:
             log.error(f"Vision Error: Could not extract text part from API response: {e}")
             st.error(f"❌ Image Analysis Error: Could not process API response structure: {e}")
             log.error("Full API Response: %s", result_json)
             return None
        ...
        if not match:
             match = re.search(r"(\{.*?\})", text_result.strip(), re.DOTALL) # Assume whole output is JSON if no markdown
        ...
        try:
            ...
            st.error(f"⚠️ Image Analysis Error: Failed to decode AI response data: {e}")
            return None
        ...
        log.exception("Unexpected error during image analysis.") # Logs traceback
...
def generate_meal_plan_with_rest(api_key: str, calorie_target: int, preferences: dict, language: str = "English"):
    ...
    try:
        ...
        payload = {
            ...
            "generationConfig": { "temperature": 0.6 } # Balance between deterministic and creative
        }
        ...
        result_json = response.json()
        if not result_json.get("candidates"):
             log.error("Meal Plan Error: No 'candidates' found in API response JSON.")
        ...
        try:
            if not result_json["candidates"][0].get("content") or not result_json["candidates"][0]["content"].get("parts"):
                 log.error("Meal Plan Error: Unexpected content/parts structure in response.")
                 st.error("❌ Meal Plan Error: Unexpected response structure from AI.")
                 log.error("Full API Response: %s", result_json)
                 return None
            text_result = result_json["candidates"][0]["content"]["parts"][0]["text"]
        ...
        try:
            ...
            if isinstance(final_plan_data, dict):
                log.info(f"Successfully extracted meal plan dictionary. Days found: {list(final_plan_data.keys())}")
                return final_plan_data # Return the dictionary {"day1": ..., "day2": ...}
            else:
                log.error("Meal Plan Error: Value under 'meal_plan' is not a dictionary (type: %s).", type(final_plan_data))
                st.error(f"❌ Meal Plan Error: AI returned unexpected data format for meal plan (expected dictionary, got {type(final_plan_data)}).")
...
def generate_grocery_list_with_rest(api_key: str, meal_plan_dict: dict, language: str = "English"):
    ...
    try:
        ...
        for day_key, day_content in meal_plan_dict.items():
            if not isinstance(day_content, dict): continue
                ...
                if isinstance(info, dict):
                    ...
                    while True:
                        ...
                        if dish_key in info:
                            ...
                            if dish_name and isinstance(dish_name, str) and dish_name.strip(): all_dishes.append(dish_name.strip())
                            i += 1; suffix = str(i)
        ...
        if not all_dishes:
            ...
            st.warning("No dish names found in the plan to create a grocery list.") # User feedback
            return None
...
"""
        ...
        try:
            if not result_json["candidates"][0].get("content") or not result_json["candidates"][0]["content"].get("parts"):
                 log.error("Grocery List Error: Unexpected content/parts structure in response.")
                 st.error("❌ Grocery List Error: Unexpected response structure from AI.")
                 log.error("Full API Response: %s", result_json)
                 return None
